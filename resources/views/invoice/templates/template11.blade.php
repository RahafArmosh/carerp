@php
    $settings_data = \App\Models\Utility::settingsById($invoice->created_by ?? \Auth::id());
    $invoiceLogo = $img ?? '';
    $companyEmail = trim(
    ...
    $invoiceNoDisplay = auth()->check()
        ? auth()->user()->invoiceNumberFormat($invoice->invoice_id)
        : (($settings['invoice_prefix'] ?? '#INVO').sprintf('%05d', $invoice->invoice_id));
            ...
            <tr>
                <td>
                    ...
                    <div class="under-title">
                        <div><strong>{{ __('Invoice No.') }}</strong> {{ $invoiceNoDisplay }}</div>
                        @if ($companyName !== '')
                ...
                <td class="logo-wrap">
                    @if (! empty($invoiceLogo))
                        <img src="{{ $invoiceLogo }}" alt="{{ __('Logo') }}">
                    @endif
                </td>
            </tr>
        </table>
            ...
            <tr>
                ...
                <td>
                    <table class="meta-table">
                        ...
                        <tr>
                            <td>{{ __('Due date') }}</td>
                            <td>{{ ! empty($invoice->due_date) ? \App\Models\Utility::dateFormat($settings, $invoice->due_date) : '—' }}
                            </td>
                        </tr>
                        @if (! empty($invoice->ref_number))
                            <tr>
                                <td>{{ __('Reference') }}</td>
                                <td>{{ $invoice->ref_number }}</td>
                            </tr>
                        @endif
                        @if (($invoice->type ?? '') === 'rent' && $invoice->getDaysDifferenceAttribute())
                            <tr>
                                <td>{{ __('Number of days') }}</td>
            ...
            <tbody>
                @if (isset($invoice->itemData) && count($invoice->itemData) > 0)
                    @foreach ($invoice->itemData as $item)
                        @php
                            $unitName = \App\Models\ProductServiceUnit::find($item->unit ?? null);
                            $unitLabel = ! empty($unitName) ? $unitName->name : '';
                            $itemtax = 0;
                            $itemLabel = trim(implode(' / ', array_filter([
                                $item->brand ?? '',
                                $item->subBrand ?? '',
                                $item->name ?? '',
                            ])));
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @endphp
                                {{ \App\Models\Utility::priceFormat($settings, ($item->price * $item->quantity) - ($item->discount ?? 0) + $itemtax) }}
                            </td>
                        </tr>
                    @endforeach
                @endif
                @if ($invoice->exists)
                    @foreach ($invoice->expenses as $expense)
                        <tr>
                            <td>{{ $expense->description ?? __('Expense') }}</td>
                            <td>{{ \App\Models\Utility::priceFormat($settings, $expense->amount) }}</td>
                            <td>1</td>
                            <td>{{ \App\Models\Utility::priceFormat($settings, $expense->amount) }}</td>
                        </tr>
                    @endforeach
                @endif
                @if ((! isset($invoice->itemData) || count($invoice->itemData) === 0) && (! $invoice->exists || $invoice->expenses->count() === 0))
                    <tr>
        ...
        <div class="totals-wrap">
            <table>
                ...
                @if (! empty($invoice->taxesData))
                    ...
                    @endforeach
                @elseif ($invoice->getTotalTax() > 0)
                    <tr>
                        <td>{{ __('Tax') }}</td>
                        <td>{{ \App\Models\Utility::priceFormat($settings, $invoice->getTotalTax()) }}</td>
                    </tr>
                @endif
                <tr class="grand">
                    <td>{{ __('Total') }}</td>
                    <td>{{ \App\Models\Utility::priceFormat($settings, $invoice->getTotal()) }}</td>
                </tr>
                <tr>
                    <td>{{ __('Paid') }}</td>
                    <td>{{ \App\Models\Utility::priceFormat($settings, $invoice->getTotalPaid()) }}</td>
                </tr>
                <tr>
                    <td>{{ __('Credit Note') }}</td>
                    <td>{{ \App\Models\Utility::priceFormat($settings, $invoice->invoiceTotalCreditNote()) }}</td>
                </tr>
                <tr>
                    <td>{{ __('Due amount') }}</td>
                    <td>{{ \App\Models\Utility::priceFormat($settings, $invoice->getDue()) }}</td>
                </tr>
            </table>
        </div>
