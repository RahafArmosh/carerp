                                    ...
                                    @if(Gate::check('edit complaint') || Gate::check('delete complaint'))
                                        <td>
                                           ...
                                           <div class="action-btn bg-danger ms-2">
                                               <form action="{{ route('complaint.destroy', $complaint->id) }}" method="POST" id="delete-form-{{ $complaint->id }}">
                                                   @csrf
                                                   @method('DELETE')
                                                   <a href="#!" class="mx-3 btn btn-sm align-items-center bs-pass-para" data-bs-toggle="tooltip" title="{{ __('Delete') }}" data-original-title="{{ __('Delete') }}" onclick="confirmDelete('{{ __('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?') }}', 'delete-form-{{ $complaint->id }}')">
