@php
    ...
    $proposalNoDisplay = \App\Models\Utility::proposalNumberFormat($settings, $proposal->proposal_id);
    $issueDate = $proposal->issue_date ?? null;
    $docHeading = ! empty($pdfTitle) ? $pdfTitle : __('Proposal');
    $custName = $customer->name ?? $customer->billing_name ?? null;

    $sumSubtotal = 0;
    $sumDiscount = 0;
    if (! empty($proposal->itemData) && count($proposal->itemData) > 0) {
        foreach ($proposal->itemData as $it) {
            $sumSubtotal += (float) $it->price * (float) $it->quantity;
            $sumDiscount += (float) ($it->discount ?? 0) * (float) $it->quantity;
        }
    }
    $sumTax = 0;
    if (! empty($proposal->taxesData) && is_array($proposal->taxesData)) {
        foreach ($proposal->taxesData as $taxPrice) {
            $sumTax += (float) $taxPrice;
        }
    } else {
        $sumTax = (float) $proposal->getTotalTax();
    }
    $grandTotal = $sumSubtotal - $sumDiscount + $sumTax;
@endphp
<!DOCTYPE html>
    ...
    <div class="page" id="boxes">
            ...
            <tr>
                <td>
                    ...
                    <div class="under-title">
                        <div><strong>{{ __('Proposal No.') }}</strong> {{ $proposalNoDisplay }}</div>
                        @if ($companyName !== '')
                ...
                <td class="logo-wrap">
                    @if (! empty($proposalLogo))
                        <img src="{{ $proposalLogo }}" alt="{{ __('Logo') }}">
                    @endif
                </td>
            </tr>
        </table>
        ...
        <table class="mid-row">
            <tr>
                <td>
                    ...
                    @if (! empty($custName) || ! empty($customer->billing_address))
                ...
                <td>
                    ...
                    @if (! empty($customFields) && ! empty($proposal->customField) && count($proposal->customField) > 0)
                        <table class="meta-table" style="margin-top:12px;">
                            @foreach ($customFields as $field)
                                <tr>
                                    <td>{{ $field->name }}</td>
                                    <td>{{ $proposal->customField[$field->id] ?? '—' }}</td>
                                </tr>
                            @endforeach
                        </table>
                    @endif
        ...
        @if (! empty($proposal->description))
            <div class="proposal-desc">
                <span class="label-strong">{{ __('Description') }}</span>
                        ...
                        <tr>
                            <td>
                                ...
                                @if (! empty($item->proposal_item_custom_fields) && is_array($item->proposal_item_custom_fields))
                                    @foreach ($item->proposal_item_custom_fields as $pair)
                                        <div class="tax-line">{{ $pair['name'] ?? '' }}: {{ $pair['value'] ?? '' }}</div>
                                    @endforeach
                                @endif
                            </td>
                            <td>{{ \App\Models\Utility::priceFormat($settings, $item->price) }}</td>
                            <td>{{ $item->quantity }}@if ($unitLabel !== '')
                                    ({{ $unitLabel }})
                                @endif
                            </td>
                            <td>
                                ...
                                @endphp
                                {{ \App\Models\Utility::priceFormat($settings, $item->price * $item->quantity - ($item->discount ?? 0) * $item->quantity + $itemtax) }}
                            </td>
                        </tr>
                    @endforeach
                @else
        ...
        <div class="totals-wrap">
            <table>
                <tr>
                    <td>{{ __('Subtotal') }}</td>
                    <td>{{ \App\Models\Utility::priceFormat($settings, $sumSubtotal) }}</td>
                </tr>
                @if ($sumDiscount > 0)
                    <tr>
                        <td>{{ __('Discount') }}</td>
                        <td>{{ \App\Models\Utility::priceFormat($settings, $sumDiscount) }}</td>
                    </tr>
                @endif
                ...
                <tr class="grand">
                    <td>{{ __('Total') }}</td>
                    <td>{{ \App\Models\Utility::priceFormat($settings, $grandTotal) }}</td>
                </tr>
            </table>
        </div>
