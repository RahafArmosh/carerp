...
@section('content')
                        ...
                        <div class="card-body">
                            <div class="row timeline-wrapper">
                                ...
                                <div class="col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    <br>
                                    <div class="float-right mt-2 col-md-3 float-end ml-5" data-toggle="tooltip"
    ...
    @if (\Auth::user()->type == 'company' || \Auth::user()->can('view proposal') || \Auth::user()->can('show proposal'))
        {{-- @if ($proposal->status != 0) --}}
        <div class="row justify-content-between align-items-center mb-3">
            <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end mb-3">
                @can('send proposal')
                    <div class="all-button-box mx-2">
                        <a href="{{ route('proposal.resent', $proposal->id) }}"
                            class="btn btn-primary">{{ __('Resend Proposal') }}</a>
                    </div>
                @endcan
                <div class="all-button-box">
                    <a href="{{ route('proposal.pdf', [Crypt::encrypt($proposal->id), 'invoice']) }}"
                        class="btn btn-primary" target="_blank">{{ __('Download Invoice') }} <i
                            class="ti ti-download"></i></a>
                    <a href="{{ route('proposal.pdf', [Crypt::encrypt($proposal->id), 'quotation']) }}"
                        class="btn btn-primary" target="_blank">{{ __('Download Quotation') }} <i
                            class="ti ti-download"></i></a>
                </div>
            </div>
            {{-- @endif --}}
        @else
            <div class="row justify-content-between align-items-center mb-3">
                <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
                    <div class="all-button-box">
                        <a href="{{ route('proposal.pdf', [Crypt::encrypt($proposal->id), 'invoice']) }}"
                            class="btn btn-xs btn-white btn-icon-only width-auto" target="_blank">{{ __('Download') }}</a>
                    </div>
                </div>
            </div>
    @endif
                            ...
                            <div class="row invoice-title mt-2">
                                ...
                                <div class="col-xs-12 col-sm-12 col-nd-6 col-lg-6 col-12 text-end">
                                    <h4 class="invoice-number">
                                        {{ Auth::user()->proposalNumberFormat($proposal->proposal_id) }}</h4>
                                </div>
                                <div class="col-12">
                                    <hr>
                                </div>
                                ...
                                <div class="col">
                                    <div class="float-end mt-3">
                                        {!! DNS2D::getBarcodeHTML(
                                            route('proposal.link.copy', \Illuminate\Support\Facades\Crypt::encrypt($proposal->id)),
                                            'QRCODE',
                                            2,
                                            2,
                                        ) !!}
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="table-responsive mt-2">
                                        @php
                                            ...
                                            $proposalItemCustomFieldsLocal = \App\Models\CustomField::where('created_by', (int) $proposal->created_by)
                                                ->where('module', 'proposal_item')
                                                ->orderBy('id')
                                                ->get(['id', 'name']);
                                            $proposalItemCustomFieldsCount = $proposalItemCustomFieldsLocal->count();

                                            $proposalItemCustomValuesByRecord = collect();
                                            $proposalItemIds = collect($iteams ?? [])
                                                ->pluck('id')
                                                ->filter()
                                                ->unique()
                                                ->values();
                                            if ($proposalItemCustomFieldsCount > 0 && $proposalItemIds->isNotEmpty()) {
                                                $proposalItemCustomValuesByRecord = \App\Models\CustomFieldValue::whereIn('record_id', $proposalItemIds->all())
                                                    ->whereIn('field_id', $proposalItemCustomFieldsLocal->pluck('id')->all())
                                                    ->whereNotNull('value')
                                            ...
                                            <thead>
                                                <tr>
                                                    ...
                                                    @if ($proposalItemCustomFieldsCount > 0)
                                                        @foreach ($proposalItemCustomFieldsLocal as $cf)
                                                            <th class="text-dark">{{ __($cf->name) }}</th>
                                                        @endforeach
                                                    @endif
                                            ...
                                            @foreach ($iteams as $key => $iteam)
                                                <tr>
                                                    ...
                                                    @php
                                                        ...
                                                        $proposalItemIdForCustom = $iteam->id ?? null;
                                                        $customValsForProposalItem = $proposalItemIdForCustom
                                                            ? ($proposalItemCustomValuesByRecord->get($proposalItemIdForCustom) ?? collect())
                                                            : collect();
                                                    @endphp
                                                    <td>
                                                    ...
                                                    @if ($proposalItemCustomFieldsCount > 0)
                                                        @foreach ($proposalItemCustomFieldsLocal as $cf)
                                                            @php
                                                                $val = optional($customValsForProposalItem->firstWhere('field_id', $cf->id))->value;
                                                            @endphp
                                                            <td>{{ ($val !== null && trim((string) $val) !== '') ? $val : '-' }}</td>
                                                        @endforeach
                                                    @endif
                                                    ...
                                                    <td class="text-end">
                                                        ...
                                                        {{ \Auth::user()->priceFormat($iteam->price * $iteam->quantity - $iteam->discount) }}
                                                    </td>
                                                </tr>
                                            @endforeach
                                            <tfoot>
                                                <tr>
                                                    ...
                                                    <td><b>-</b></td>
                                                    @if ($proposalItemCustomFieldsCount > 0)
                                                        @for ($c = 0; $c < $proposalItemCustomFieldsCount; $c++)
                                                            <td></td>
                                                        @endfor
                                                    @endif
                                                    <td></td>
                                                ...
                                                <tr>
                                                    <td colspan="{{ 6 + $proposalItemCustomFieldsCount }}"></td>
                                                ...
                                                @php
                                                    // Calculate proposal-level tax from tax_id
                                                    $proposalTax = 0;
                                                    $proposalTaxesData = [];
                                                    if (!empty($proposal->tax_id)) {
                                                        $getTaxData = Utility::getTaxData();
                                                        $taxArr = explode(',', $proposal->tax_id);
                                                        $taxableAmount = $proposal->getSubTotal() - $proposal->getTotalDiscount();
                                                        
                                                        foreach ($taxArr as $taxId) {
                                                            if (!empty($getTaxData[$taxId])) {
                                                                $taxInfo = $getTaxData[$taxId];
                                                                $vatType = $taxInfo['type'] ?? 'add';
                                                                $taxAmount = 0;
                                                                
                                                                if ($vatType === 'add') {
                                                                    $taxAmount = $taxableAmount * ($taxInfo['rate'] / 100);
                                                                } else if ($vatType === 'subtract') {
                                                                    $taxAmount = ($taxableAmount * ($taxInfo['rate'] / 100)) / (1 + ($taxInfo['rate'] / 100));
                                                                }
                                                                
                                                                $proposalTax += $taxAmount;
                                                                $proposalTaxesData[$taxInfo['name']] = [
                                                                    'rate' => $taxInfo['rate'],
                                                                    'type' => $vatType,
                                                                    'amount' => $taxAmount
                                                                ];
                                                ...
                                                @if ($proposalTax > 0 && !empty($proposalTaxesData))
                                                    @foreach ($proposalTaxesData as $taxName => $taxInfo)
                                                        <tr>
                                                            <td colspan="{{ 6 + $proposalItemCustomFieldsCount }}"></td>
                                                            <td class="text-end"><b>{{ $taxName }} ({{ $taxInfo['rate'] }}%)</b></td>
                                                            <td class="text-end">
                                                                {{ \Auth::user()->priceFormat($taxInfo['amount']) }}</td>
                                                        </tr>
                                                    @endforeach
                                                @endif
                                                <tr>
                                                    ...
                                                    <td class="blue-text text-end">
                                                        {{ \Auth::user()->priceFormat($proposal->getSubTotal() - $proposal->getTotalDiscount() + $proposalTax) }}</td>
                                                </tr>
                                            </tfoot>
                                        </table>
