<form method="POST" action="{{ route('resignation.update', $resignation->id) }}">
    @csrf
    @method('PUT')
    <div class="modal-body">
        ...
        <div class="row">
                ...
                <div class="form-group col-lg-12">
                    ...
                    <select name="employee_id" id="employee_id" class="form-control select" required>
                        @foreach ($employees as $id => $name)
                            <option value="{{ $id }}"
                                {{ $resignation->employee_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
                </div>
            @endif
            ...
            <div class="form-group col-lg-12">
                ...
                <textarea name="description" id="description" class="form-control" placeholder="{{ __('Enter Description') }}">{{ $resignation->description }}</textarea>
            </div>
        </div>
    </div>
