...
@push('script-page')
    ...
    <script>
        // Provide template descriptions to the client
        const templateDescriptions = @json(App\Models\Utility::templateData()['descriptions']);
        ...
        $(document).on("change", "select[name='invoice_template'], input[name='invoice_color']", function() {
            ...
            // Update description
            $('#invoice_template_description').text((templateDescriptions.invoice && templateDescriptions.invoice[
                template]) || 'Select a template to see description');
        });
        ...
        $(document).on("change", "select[name='proposal_template'], input[name='proposal_color']", function() {
            ...
            // Update description
            $('#proposal_template_description').text((templateDescriptions.proposal && templateDescriptions
                .proposal[template]) || 'Select a template to see description');
        });
        ...
        $(document).on("change", "select[name='bill_template'], input[name='bill_color']", function() {
            ...
            // Update description
            $('#bill_template_description').text((templateDescriptions.bill && templateDescriptions.bill[
                template]) || 'Select a template to see description');
        });

        // Initialize descriptions on load
        $(function() {
            $("select[name='invoice_template']").trigger('change');
            $("select[name='proposal_template']").trigger('change');
            $("select[name='bill_template']").trigger('change');
        });
    ...
    <div class="col-sm-12 mt-4">
        <div class="card">
            <div class="card-body">
                ...
                <ul class="nav nav-pills mb-3 nav-pills-outline" id="pills-tab" role="tablist">
                    <li class="nav-item">
                ...
                <div class="tab-content" id="pills-tabContent">
                    ...
                    <div class="tab-pane fade show active" id="pills-proposal" role="tabpanel"
                        ...
                        @php
                            $proposalTemplateResolved = \App\Models\Utility::resolveProposalTemplate(
                                $settings['proposal_template'] ?? null,
                            );
                        @endphp
                                        ...
                                        <form id="setting-form" method="post"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control select2" name="proposal_template">
                                                    @foreach (App\Models\Utility::templateData()['templates']['proposal'] as $key => $template)
                                                        <option value="{{ $key }}"
                                                            {{ isset($settings['proposal_template']) && \App\Models\Utility::resolveProposalTemplate($settings['proposal_template']) === $key ? 'selected' : '' }}>
                                                            {{ $template }}</option>
                                                    @endforeach
                                                </select>
                                                <div class="template-description mt-2">
                                                    <small class="text-dark" id="proposal_template_description">
                                                        @if (isset(App\Models\Utility::templateData()['descriptions']['proposal'][$proposalTemplateResolved]))
                                                            {{ App\Models\Utility::templateData()['descriptions']['proposal'][$proposalTemplateResolved] }}
                                                        @else
                                ...
                                <div class="col-md-9">
                                    @if (isset($settings['proposal_template']) && isset($settings['proposal_color']))
                                        <iframe id="proposal_frame" class="w-100 h-100" frameborder="0"
                                            src="{{ route('proposal.preview', [$proposalTemplateResolved, $settings['proposal_color']]) }}"></iframe>
                                    @else
                                        <iframe id="proposal_frame" class="w-100 h-100" frameborder="0"
                    ...
                    <div class="tab-pane fade" id="pills-invoice" role="tabpanel" aria-labelledby="pills-invoice-tab">
                        @php
                            $invoiceTemplateResolved = \App\Models\Utility::resolveInvoiceTemplate(
                                $settings['invoice_template'] ?? null,
                            );
                        @endphp
                                        ...
                                        <form id="setting-form" method="post" action="{{ route('template.setting') }}"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control select2" name="invoice_template">
                                                    @foreach (Utility::templateData()['templates']['invoice'] as $key => $template)
                                                        <option value="{{ $key }}"
                                                            {{ isset($settings['invoice_template']) && \App\Models\Utility::resolveInvoiceTemplate($settings['invoice_template']) === $key ? 'selected' : '' }}>
                                                            {{ $template }}</option>
                                                    @endforeach
                                                </select>
                                                <div class="template-description mt-2">
                                                    <small class="text-dark" id="invoice_template_description">
                                                        @if (isset(App\Models\Utility::templateData()['descriptions']['invoice'][$invoiceTemplateResolved]))
                                                            {{ App\Models\Utility::templateData()['descriptions']['invoice'][$invoiceTemplateResolved] }}
                                                        @else
                                                            {{ 'Select a template to see description' }}
                                                        @endif
                                                    </small>
                                ...
                                <div class="col-md-9">
                                    @if (isset($settings['invoice_template']) && isset($settings['invoice_color']))
                                        <iframe id="invoice_frame" class="w-100 h-100" frameborder="0"
                                            src="{{ route('invoice.preview', [$invoiceTemplateResolved, $settings['invoice_color']]) }}"></iframe>
                                    @else
                                        <iframe id="invoice_frame" class="w-100 h-100" frameborder="0"
                                        ...
                                        <form id="setting-form" method="post"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="bill_template">
                                                    @foreach (App\Models\Utility::templateData()['templates']['bill'] as $key => $template)
                                                        <option value="{{ $key }}"
                                                ...
                                                <div class="template-description mt-2">
                                                    <small class="text-dark" id="bill_template_description">
                                                        @if (isset($settings['bill_template']) &&
                                                                isset(App\Models\Utility::templateData()['descriptions']['bill'][$settings['bill_template']]))
                                                            {{ App\Models\Utility::templateData()['descriptions']['bill'][$settings['bill_template']] }}
                                                        @else
                                                            {{ 'Select a template to see description' }}
                                                        @endif
                                                    </small>
