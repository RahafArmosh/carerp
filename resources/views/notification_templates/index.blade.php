                ...
                <div class="d-flex justify-content-end drp-languages">
                    <ul class="list-unstyled mb-0 m-2 me-0">
                        <li class="dropdown dash-h-item drp-language">
                            <a class="email-color dash-head-link dropdown-toggle arrow-none me-0" data-bs-toggle="dropdown"
                                ...
                                id="dropdownLanguage">
                                <span class="drp-text hide-mob text-primary me-2">{{ ucfirst($LangName->full_name) }}</span>
                    ...
                    @if ($plan->chatgpt == 1)
                        <ul class="list-unstyled mb-0 mt-3">
                            <div class="">
                                <a href="#" data-size="md" class="btn  btn-primary btn-sm" data-ajax-popup-over="true"
        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-body ">
                    ...
                    <form method="POST"
                        action="{{ route('notification-templates.update', $curr_noti_tempLang->parent_id) }}">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="form-group col-12">
                                <label for="content" class="form-label text-dark">{{ __('Notification Message') }}</label>
                                <textarea id="content" name="content" class="form-control" required rows="4"
                                    placeholder="EX. Hello, {company_name}">{{ $curr_noti_tempLang->content }}</textarea>
                                <small>{{ __('A variable is to be used in such a way.') }} <span
                                        class="text-primary">{{ __('Ex. Hello, {user_name}') }}</span></small>
                            </div>
                        </div>
                        <hr>
