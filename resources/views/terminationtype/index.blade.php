    ...
    <div class="float-end">
        @can('create termination type')
            <a href="#" data-url="{{ route('terminationtype.create') }}" data-ajax-popup="true"
                data-title="{{ __('Create New Termination Type') }}" data-bs-toggle="tooltip" title="{{ __('Create') }}"
                                        ...
                                        <td>
                                            ...
                                            @can('delete termination type')
                                                <div class="action-btn bg-danger ms-2">
                                                    <form id="delete-form-{{ $terminationtype->id }}"
                                                        action="{{ route('terminationtype.destroy', $terminationtype->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <a href="#"
                                                            ...
                                                            onclick="event.preventDefault(); if(confirm('{{ __('Are You Sure?') }}')) document.getElementById('delete-form-{{ $terminationtype->id }}').submit();">
