@php
    ...
    $parts = array_filter([
        ...
        $payment->description && $payment->description !== 'NULL' ? $payment->description : null,
    ]);
    if ($linkedBill) {
        array_unshift($parts, __('Bill').' '.Utility::billNumberFormat($settings, $linkedBill->bill_id));
    }
    $desc = count($parts) ? implode(' | ', $parts) : __('Vendor refund');
...
<html lang="en" dir="{{ ($settings_data['SITE_RTL'] ?? $settings['SITE_RTL'] ?? '') == 'on' ? 'rtl' : 'ltr' }}">
    ...
    <head>
        ...
        <title>{{ __('Refund') }} {{ $refundNo }}</title>
    ...
    <body>
        <div class="doc" id="boxes">
            <header class="doc-header">
                <h1 class="doc-title">{{ __('Refund') }}</h1>
                <div class="doc-brand">
            ...
            @if (! empty($settings['company_name']) || ! empty($settings['footer_notes']) || ! empty($settings['refund_terms'] ?? ''))
                <div class="doc-footnote">
                    ...
                    @endif
                    @if (! empty($settings['refund_terms'] ?? ''))
                        <div class="refund-terms">{!! nl2br(e($settings['refund_terms'])) !!}</div>
                    @endif
                </div>
            @endif
        </div>
        ...
        <script>
            ...
            $(window).on('load', function() {
                ...
                var opt = {
                    margin: 0.5,
                    filename: 'Refund-{{ $payment->id }}.pdf',
                    image: {
                        type: 'jpeg',
                        quality: 1
                    },
