...
@section('action-btn')
    <div class="float-end">
        ...
        @endcan
        <a href="{{ route('product-category.export') }}" data-bs-toggle="tooltip" title="{{ __('Export') }}"
                    ...
                    <div class="table-responsive">
                        <table class="table datatable">
                            ...
                            @foreach ($categories as $category)
                                <tr>
                                    <td class="font-style">{{ $category->id }}</td>
                                    <td class="font-style">{{ $category->name }}</td>
                                    <td class="font-style">
                                        {{ __(\App\Models\ProductServiceCategory::$catTypes[$category->type]) }}
                                    </td>
                                    <td>{{ (!empty($category->saleAccount)?$category->saleAccount->name :'-') }}</td>
                                    <td>{{ (!empty($category->expenseAccount)?$category->expenseAccount->name :'-') }}</td>
                                    <td>{{ ($category->purchase_account_id !== 0 ? \App\Models\ChartOfAccount::where('id',$category->purchase_account_id)->first()->name :'-') }}</td>
                                    <td>
                                        {{ $category->is_manufacturer ? "Yes" : "No" }}
                                    </td>
