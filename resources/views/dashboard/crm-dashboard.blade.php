...
@endsection
@section('content')
    <div class="motor-surface-flat mb-4 rounded-motor border border-dashed border-motor-border bg-motor-elevated/70 px-5 py-4 shadow-none">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="max-w-xl">
                <p class="motor-xs font-semibold uppercase tracking-[0.12em] text-motor-accent">{{ __('CRM') }}</p>
                <h2 class="mt-1 font-sans text-xl font-semibold tracking-tight text-motor-ink">{{ __('Dealership pipeline clarity') }}</h2>
                <p class="mt-2 motor-sm leading-relaxed text-motor-muted">
                    {{ __('Leads → deals → contracts in one calibrated surface. Below cards stay wired to live CRM data.') }}</p>
            </div>
            <span class="inline-flex items-center gap-2 rounded-full bg-motor-canvas px-3 py-1.5 motor-sm font-semibold text-motor-ink">{{ __('Synced') }}</span>
        </div>
    </div>
    <div class="row">
