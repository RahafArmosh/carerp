...
<head>
    <title>
        {{ Utility::getValByName('title_text') ? Utility::getValByName('title_text') : config('app.name', 'Orbix') }}
        - Form Builder</title>
                ...
                <div class="row justify-content-center">
                    <div class="col-sm-8 col-lg-5">
                        ...
                        <div class="card shadow zindex-100 mb-0">
                            @if ($form->is_active == 1)
                                <form action="{{ route('form.view.store') }}" method="POST">
                                    @csrf
                                            ...
                                            @foreach ($objFields as $objField)
                                                @if ($objField->type == 'text')
                                                    <div class="form-group">
                                                        <label for="field-{{ $objField->id }}"
                                                            class="form-label">{{ __($objField->name) }}</label>
                                                        <input type="text" name="field[{{ $objField->id }}]"
                                                            id="field-{{ $objField->id }}" class="form-control"
                                                            required>
                                                    </div>
                                                @elseif($objField->type == 'email')
                                                    <div class="form-group">
                                                        <label for="field-{{ $objField->id }}"
                                                            class="form-label">{{ __($objField->name) }}</label>
                                                        <input type="email" name="field[{{ $objField->id }}]"
                                                            id="field-{{ $objField->id }}" class="form-control"
                                                            required>
                                                    </div>
                                                @elseif($objField->type == 'number')
                                                    <div class="form-group">
                                                        <label for="field-{{ $objField->id }}"
                                                            class="form-label">{{ __($objField->name) }}</label>
                                                        <input type="number" name="field[{{ $objField->id }}]"
                                                            id="field-{{ $objField->id }}" class="form-control"
                                                            required>
                                                    </div>
                                                @elseif($objField->type == 'date')
                                                    <div class="form-group">
                                                        <label for="field-{{ $objField->id }}"
                                                            class="form-label">{{ __($objField->name) }}</label>
                                                        <input type="date" name="field[{{ $objField->id }}]"
                                                            id="field-{{ $objField->id }}" class="form-control"
                                                            required>
                                                    </div>
