...
@section('content')
                            ...
                            <ul class="list-inline mb-0">
                                @can('delete job application')
                                    <li class="list-inline-item float-end">
                                        <form method="POST"
                                            action="{{ route('job.application.archive', $jobApplication->id) }}"
                                            ...
                                            <a href="#"
                                                ...
                                                @else
                                                    <span
                                                        class="badge bg-warning p-2 px-3 rounded">{{ __('UnArchive') }}</span>
                                                @endif
                                            </a>
                                        </form>
                                    ...
                                    @if ($jobApplication->is_archive == 0)
                                        <li class="list-inline-item">
                                            <form method="POST"
                                                action="{{ route('job-application.destroy', $jobApplication->id) }}"
            ...
            <div class="card ">
                ...
                <div class="card-body">
                    <dl class="row">
                        ...
                        <dd class="col-sm-9"><span
                                class="text-sm">{{ \Auth::user()->dateFormat($jobApplication->created_at) }}</span></dd>
                            ...
                            @if (!empty($jobApplication->resume))
                                <span class="text-sm action-btn bg-primary ms-2 ">
                                    <a href="{{ asset(Storage::url('uploads/job/resume')) . '/' . $jobApplication->resume }}"
                                        download="" target="_blank"><i class="ti ti-download text-white"></i></a>
                                </span>
                            @else
                                -
                            @endif
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            @endif
            <form method="POST" action="{{ route('job.application.skill.store', $jobApplication->id) }}">
                @csrf
            ...
            <form method="POST" action="{{ route('job.application.note.store', $jobApplication->id) }}">
                @csrf
                <div class="form-group">
                            ...
                            @can('delete job application note')
                                @if ($note->note_created == \Auth::user()->id)
                                    <div class="action-btn bg-danger ms-2">
                                        <form method="POST" action="{{ route('job.application.note.destroy', $note->id) }}"
                                            id="delete-form-{{ $note->id }}">
                                            @csrf
                                            @method('DELETE')

                                            <a class="mx-3 btn btn-sm align-items-center bs-pass-para" href="#"
                                                data-confirm="Are You Sure?|This action can not be undone. Do you want to continue?"
