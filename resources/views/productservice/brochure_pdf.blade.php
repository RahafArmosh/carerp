...
<head>
    <meta charset="utf-8">
    <title>{{ $productService->name }} — {{ __('Product brochure') }}</title>
    <style>
        * {
...
<body>
    <table class="header">
        <tr>
            ...
            <td>
                <div class="tag">{{ __('Product brochure') }}</div>
                <div class="brand">{{ $settings['company_name'] ?? config('app.name', 'AutoCore') }}</div>
                <h1>{{ $productService->name }}</h1>
                <div class="sku">{{ __('SKU') }}: {{ $productService->sku }}</div>
            </td>
        </tr>
    </table>
    ...
    <table class="grid">
        ...
        <tr>
            ...
            <td>{{ optional($productService->brand)->name ?? '—' }}</td>
        </tr>
        <tr>
            <td class="label">{{ __('Model') }}</td>
            <td>{{ optional($productService->subBrand)->name ?? '—' }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>{{ ucfirst($productService->type) }}</td>
        </tr>
        <tr>
            <td class="label">{{ __('Sale price') }}</td>
            <td>{{ number_format((float) $productService->sale_price, 2) }}</td>
        </tr>
        <tr>
            <td class="label">{{ __('Purchase price') }}</td>
            <td>{{ number_format((float) $productService->purchase_price, 2) }}</td>
        </tr>
    </table>
    ...
    @if (count($imageBlocks) > 0)
        <div class="section-title">{{ __('Images') }}</div>
        <div class="images">
