            ...
            <div class="form-group">
                ...
                <select name="trainer_option" class="form-control select" required>
                    @foreach ($options as $option)
                        <option value="{{ $option }}" {{ old('trainer_option') == $option ? 'selected' : '' }}>
                            {{ $option }}</option>
                    @endforeach
                </select>
            </div>
                ...
                <select name="training_type" class="form-control select" required>
                    @foreach ($trainingTypes as $trainingType)
                        <option value="{{ $trainingType }}"
                            {{ old('training_type') == $trainingType ? 'selected' : '' }}>{{ $trainingType }}</option>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <div class="form-group">
                ...
                <select name="trainer" class="form-control select" required>
                    @foreach ($trainers as $trainer)
                        <option value="{{ $trainer }}" {{ old('trainer') == $trainer ? 'selected' : '' }}>
                            {{ $trainer }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <input type="number" name="training_cost" class="form-control" step="0.01"
                    value="{{ old('training_cost') }}" required>
            </div>
        </div>
        <div class="col-md-12">
            <div class="form-group">
