...
@section('action-btn')
    ...
    <div class="float-end me-2">
        <form method="POST" action="{{ route('balance.sheet.export') }}">
            @csrf
            ...
            <div class="col-md-8">
                <div class="mt-2" id="multiCollapseExample1">
                    <div class="card" id="show_filter" style="display:none;">
                        <div class="card-body">
                            <form method="GET" action="{{ route('report.balance.sheet') }}" id="report_bill_summary">
                                <div class="row align-items-center justify-content-end">
                                    <div class="col-xl-10">
                                        <div class="row">
                                            ...
                                            <div class="col-xl-3 col-lg-3 col-md-6 col-sm-12 col-12">
                                                <div class="btn-box">
                                                    <label for="start_date"
                                                        class="form-label">{{ __('From Date') }}</label>
                                            ...
                                            <div class="col-xl-3 col-lg-3 col-md-6 col-sm-12 col-12">
                                                <div class="btn-box">
                                                    <label for="end_date" class="form-label">{{ __('As Date') }}</label>
                            ...
                            @if ($accounts != [])
                                <div class="account-main-inner py-2">
                                    ...
                                    @foreach ($accounts as $account)
                                        <div class="border-bottom py-2">
                                                    ...
                                                    @if (!preg_match('/\btotal\b/i', $record['account_name']))
                                                        <div
                                                            ...
                                                            <p class="text-primary mb-2 float-end text-end">
                                                                {{ \Auth::user()->priceFormat($record['netAmount'] * -1) }}</p>
                                                        </div>
                                                    @endif
                                                @endif
                                            @endforeach
                                            <div
                                                ...
                                                <p class="fw-bold mb-2 text-end">
                                                    {{ end($account['account']) == true ? \Auth::user()->priceFormat(end($account['account'])['netAmount'] * -1) : \Auth::user()->priceFormat(0) }}
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        @php
                                            $total +=
                                                end($account['account']) == true
                                                    ? end($account['account'])['netAmount']
                                                    : 0;
                                        @endphp
                                    @endforeach
                                    <div
                                        ...
                                        <h6 class="fw-bold mb-0 text-end">{{ \Auth::user()->priceFormat($total * -1) }}</h6>
                                    </div>
                                    
                                    @if ($type == 'Liabilities')
                                        <div
                                            class="aacount-title d-flex align-items-center justify-content-between border-bottom py-2 px-2 pe-0">
                                            <h6 class="fw-bold mb-0">{{ __('Profit/Loss for the Period') }}</h6>
                                            <h6 class="fw-bold mb-0 text-end {{ $profitLoss >= 0 ? 'text-success' : 'text-danger' }}">
                                                {{ \Auth::user()->priceFormat($profitLoss * -1) }}
                                            </h6>
                                        </div>
                                        @php
                                            $totalAmount += $total + $profitLoss;
                                        @endphp
                                    @else
                                        @php
                                            if ($type != 'Assets') {
                        ...
                        @foreach ($totalAccounts as $type => $accounts)
                            ...
                            @if ($accounts != [])
                                <div
                                    ...
                                    <h6 class="fw-bold mb-0">{{ 'Total for Liabilities and Profit/Loss for the Period' }}</h6>
                                    <h6 class="fw-bold mb-0 text-end">{{ \Auth::user()->priceFormat($totalAmount * -1) }}</h6>
                                </div>
                            @endif
                            @php
