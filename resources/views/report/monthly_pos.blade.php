...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="mt-2 ">
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('report.monthly.pos') }}" method="GET" id="monthly_pos_report_submit">
                                        ...
                                        <select name="warehouse" id="warehouse" class="form-control select">
                                            @foreach ($warehouse as $warehouseItem)
                                                <option value="{{ $warehouseItem->id }}"
                                                    {{ isset($_GET['warehouse']) && $_GET['warehouse'] == $warehouseItem->id ? 'selected' : '' }}>
                                                    {{ $warehouseItem->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="customer" id="customer" class="form-control select">
                                            @foreach ($customer as $customerItem)
                                                <option value="{{ $customerItem->id }}"
                                                    {{ isset($_GET['customer']) && $_GET['customer'] == $customerItem->id ? 'selected' : '' }}>
                                                    {{ $customerItem->name }}
                                                </option>
                                            @endforeach
                                        </select>
    ...
    <div id="printableArea">
        <div class="row mt-0">
            <div class="col">
                <input type="hidden"
                    value="{{ $filter['warehouse'] . ' ' . __('Monthly Pos') . ' ' . 'Report of' . ' ' . $filter['startMonth'] . ' to ' . $filter['endMonth'] }}"
                    id="filename">
