...
@section('action-btn')
    ...
    <div class="float-end me-2">
        <form action="{{ route('trial.balance.total.export') }}" method="POST">
            @csrf
        ...
        <div class="col-sm-8">
            <div class="mt-2 " id="multiCollapseExample1">
                <div class="card" id="show_filter" style="display:none;">
                    <div class="card-body">
                        <form action="{{ route('trial.balance.total') }}" method="GET" id="report_trial_balance">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div
                        ...
                        <h6 class="mb-0 text-end me-5">{{ _('Previous Balance') }}</h6>
                        <h6 class="mb-0 text-end me-5">{{ __('Debit') }}</h6>
                        <h6 class="mb-0 text-end">{{ __('Credit') }}</h6>
                        <h6 class="mb-0 text-end">{{ __('New Balance') }}</h6>
                    ...
                    @foreach ($totalAccounts as $type => $accounts)
                        <div class="account-main-inner border-bottom py-2">
                            ...
                            @php
                                $Previous = $record['prevDebit'] - $record['prevCredit'] ;
                                $New = $Previous + ($record['totalDebit'] - $record['totalCredit']);
                            @endphp
                                <div class="account-inner d-flex align-items-center justify-content-between">
                                    <p class="mb-2"><a
                                            href="{{ route('report.ledger', $record['id']) }}?start_date={{ $filter['startDateRange'] }}&end_date={{ $filter['endDateRange'] }}&account={{ $record['id'] }}"
                                            class="text-primary">{{ $record['name'] }}</a>
                                    </p>
                                    <p class="mb-2 text-center">{{ $record['code'] }}</p>
                                    <p class="text-primary mb-2 text-center">
                                        {{ \Auth::user()->priceFormat($Previous) }}</p>
                    ...
                    @if ($totalAccounts != [])
                        <div
                            ...
                            <h6 class="fw-bold mb-0 text-end me-5">{{ \Auth::user()->priceFormat($totalPrevious) }}</h6>
                            ...
                            <h6 class="fw-bold mb-0 text-end">{{ \Auth::user()->priceFormat($totalNew) }}</h6>
