        ...
        function getEmployee(did) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $("#employee_div").html('');
                    $('#employee_div').append(
                        '<label for="employee" class="form-label">{{ __('Employee') }}</label><select class="form-control" id="employee_id" name="employee_id"></select>'
                        );
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class=" mt-2 " id="multiCollapseExample1">
                <div class="card">
                    <div class="card-body">
                        <form id="report_payroll" action="{{ route('report.payroll') }}" method="get">
                            <div class="row align-items-center justify-content-end">
                                ...
                                <div class="col-2 month">
                                    <div class="btn-box">
                                        ...
                                        <select class="month-btn form-control" id="month" name="month">
                                            @if (isset($_GET['month']))
                                                <option value="{{ $_GET['month'] }}" selected>{{ $_GET['month'] }}
                                                </option>
                                            @else
                                ...
                                <div class="col-3">
                                    <div class="btn-box" id="employee_div">
                                        <label class="form-label" for="employee">{{ __('Employee') }}</label>
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-12 col-md-4 col-lg-3 col-xl-3">
                <div class="card p-4 mb-4">
                    ...
                    <h7 class="text-sm mb-0">{{ \Auth::user()->priceFormat($filterData['totalSaturationDeduction']) }}
                    </h7>
                </div>
            </div>
            <div class="col-xs-12 col-sm-12 col-md-4 col-lg-3 col-xl-3">
                                ...
                                @foreach ($payslips as $payslip)
                                    <tr>
                                        <td><a href="{{ route('employee.show', \Illuminate\Support\Facades\Crypt::encrypt($payslip->employee_id)) }}"
                                                class="btn btn-sm btn-outline-primary">{{ !empty($payslip->employees) ? \Auth::user()->employeeIdFormat($payslip->employees->employee_id) : '' }}</a>
                                        </td>
