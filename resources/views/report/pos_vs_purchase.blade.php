    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="mt-2 " id="multiCollapseExample1">
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('report.pos.vs.purchase') }}" method="GET" id="pos_vs_purchase">
                                            ...
                                            <div class="btn-box">
                                                ...
                                                <select name="year" id="year" class="form-control select">
                                                    @foreach ($yearList as $key => $year)
                                                        <option value="{{ $key }}"
                                                            {{ isset($_GET['year']) && $_GET['year'] == $key ? 'selected' : '' }}>
                                                            {{ $year }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                    ...
                                    <div class="row">
                                        <div class="col-auto mt-4">
                                            <a href="#" class="btn btn-sm btn-primary"
                                                onclick="document.getElementById('pos_vs_purchase').submit(); return false;"
                                                data-bs-toggle="tooltip" title="{{ __('Apply') }}"
                                                data-original-title="{{ __('apply') }}">
                                                <span class="btn-inner--icon"><i class="ti ti-search"></i></span>
                                            </a>
                                            <a href="{{ route('report.pos.vs.purchase') }}" class="btn btn-sm btn-danger "
        ...
        <div id="printableArea">
            <div class="row mt-0">
                <div class="col">
                    <input type="hidden"
                        value="{{ __('POS Vs Purchase') . ' ' . 'Report of' . ' ' . $filter['startDateRange'] . ' to ' . $filter['endDateRange'] }}"
                        id="filename">
