                                                ...
                                                <div class="avatar-group">
                                                    @foreach ($item->users($item->user_id) as $projectUser)
                                                        <img alt="image" data-bs-toggle="tooltip" data-bs-placement="top"
                                                            ...
                                                            @if ($projectUser->avatar) src="{{ $profile . '/' . $projectUser->avatar }}"
                                                     @else
                                                        src="{{ $profile . 'avatar.png' }}" @endif
                                                            class="avatar rounded-circle avatar-sm avatar-group"
                                                            width="25" height="25">
                                                    @endforeach
                                                </div>
                                            @else
                                        ...
                                        <td>
                                            @if ($item->created_by == \Auth::user()->id && $item->checkDateTime())
                                                <a href="{{ $item->start_url }}" target="_blank">
                                                    {{ __('Start meeting') }} <i
                                                        class="ti ti-external-link-square-alt "></i></a>
                                            @elseif($item->checkDateTime())
                                                <a href="{{ $item->join_url }}" target="_blank"> {{ __('Join meeting') }}
                                                    <i class="ti ti-external-link-square-alt "></i></a>
                                            @else
                                                -
                                            @endif
                                        ...
                                        <td>
                                            @if ($item->checkDateTime())
                                                @if ($item->status == 'waiting')
                                                    <span
                                                        class="badge bg-info p-2 px-3 rounded status_badge">{{ ucfirst($item->status) }}</span>
                                                @else
                                                    <span
                                                        class="badge bg-success p-2 px-3 rounded status_badge">{{ ucfirst($item->status) }}</span>
                                                @endif
                                            @else
                                                <span
                                                    class="badge bg-danger p-2 px-3 rounded status_badge">{{ __('End') }}</span>
                                            @endif
                                        </td>
                                        @if (\Auth::user()->type == 'company')
                                            <td class="text-end">
                                                <div class="action-btn bg-danger ms-2">
                                                    <form id="delete-form-{{ $item->id }}"
                                                        action="{{ route('zoom-meeting.destroy', $item->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <a href="#"
