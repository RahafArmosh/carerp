                                        ...
                                        <td>
                                            ...
                                            @can('delete deduction option')
                                                <div class="action-btn bg-danger ms-2">
                                                    <form action="{{ route('deductionoption.destroy', $deductionoption->id) }}"
                                                        method="POST" id="delete-form-{{ $deductionoption->id }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <a href="#"
                                                            class="mx-3 btn btn-sm align-items-center bs-pass-para"
                                                            data-bs-toggle="tooltip" title="{{ __('Delete') }}"
                                                            onclick="confirmDelete('{{ __('Are You Sure?') }}', 'delete-form-{{ $deductionoption->id }}')">
