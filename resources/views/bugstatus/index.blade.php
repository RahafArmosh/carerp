...
@section('action-btn')
    @can('create bug status')
        <div class="float-end">
            <a href="#" data-url="{{ route('bugstatus.create') }}" data-bs-toggle="tooltip" title="{{ __('Create') }}"
                class="btn btn-sm btn-primary" data-ajax-popup="true" data-title="{{ __('Create Bug Stage') }}">
                                            ...
                                            <span class="float-end">
                                                ...
                                                @can('delete bug status')
                                                    <div class="action-btn bg-danger ms-2">
                                                        <form action="{{ route('bugstatus.destroy', $bug->id) }}"
                                                            method="POST" id="delete-form-{{ $bug->id }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <a href="#!"
                                                                ...
                                                                onclick="confirmDelete('{{ __('Are You Sure?') . '|' . __('This action can not be undone. Do you want to continue?') }}', 'delete-form-{{ $bug->id }}')">
