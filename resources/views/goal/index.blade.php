...
@section('action-btn')
    <div class="float-end">
        @can('create goal')
            <a href="#" data-url="{{ route('goal.create') }}" data-bs-toggle="tooltip" data-size="lg"
                title="{{ __('Create') }}" data-ajax-popup="true" data-title="{{ __('Create New Goal') }}"
                                                ...
                                                @can('delete goal')
                                                    <div class="action-btn bg-danger ms-2">
                                                        <form id="delete-form-{{ $goal->id }}"
                                                            action="{{ route('goal.destroy', $goal->id) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <a href="#"
                                                                class="mx-3 btn btn-sm align-items-center bs-pass-para"
                                                                data-bs-toggle="tooltip" title="{{ __('Delete') }}"
                                                                onclick="confirmDelete('{{ __('Are You Sure?') }}', event, 'delete-form-{{ $goal->id }}')">
