    ...
    <div class="float-end">
        @can('create termination')
            <a href="#" data-url="{{ route('termination.create') }}" data-size="lg" data-ajax-popup="true"
                data-title="{{ __('Create New Termination') }}" data-bs-toggle="tooltip" title="{{ __('Create') }}"
                                            ...
                                            <td>
                                                ...
                                                @can('edit termination')
                                                    <div class="action-btn bg-primary ms-2">
                                                        <a href="#" class="mx-3 btn btn-sm align-items-center"
                                                            ...
                                                            data-title="{{ __('Edit Termination') }}" data-bs-toggle="tooltip"
                                                            title="{{ __('Edit') }}"
                                                ...
                                                @can('delete termination')
                                                    <div class="action-btn bg-danger ms-2">
                                                        <form id="delete-form-{{ $termination->id }}"
                                                            action="{{ route('termination.destroy', $termination->id) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <a href="#"
                                                                ...
                                                                onclick="event.preventDefault(); if(confirm('{{ __('Are You Sure?') }} | {{ __('This action can not be undone. Do you want to continue?') }}')) document.getElementById('delete-form-{{ $termination->id }}').submit();">
