@php
    ...
    if (! $linkedInvoice && $payment->invoicePayments->isNotEmpty()) {
        $linkedInvoice = $payment->invoicePayments->first()->invoice;
    }
    $paymentNo = auth()->check()
        ? auth()->user()->CustomerpaymentNumberFormat($payment->payment_number ?? $payment->id)
        : 'CP'.sprintf('%05d', $payment->payment_number ?? $payment->id);

    $lineRows = [];
    if ($payment->invoicePayments->count() > 0) {
        foreach ($payment->invoicePayments as $invPayment) {
            $inv = $invPayment->invoice;
            $desc = $inv
                ? __('Invoice').' '.Utility::invoiceNumberFormat($settings, $inv->invoice_id)
                : __('Allocation');
            if ($invPayment->description && $invPayment->description !== 'NULL') {
                $desc .= ' — '.$invPayment->description;
            }
            $amt = (float) $invPayment->amount;
            $lineRows[] = ['desc' => $desc, 'rate' => $amt, 'qty' => 1, 'total' => $amt];
        }
        $allocatedSum = collect($lineRows)->sum('total');
        $remainder = (float) $payment->amount - $allocatedSum;
        if ($remainder > 0.009) {
            $lineRows[] = [
                'desc' => __('Remaining to allocate'),
                'rate' => $remainder,
                'qty' => 1,
                'total' => $remainder,
            ];
        ...
        $desc = count($parts) ? implode(' | ', $parts) : __('Customer payment');
...
<html lang="en" dir="{{ ($settings_data['SITE_RTL'] ?? $settings['SITE_RTL'] ?? '') == 'on' ? 'rtl' : 'ltr' }}">
    ...
    <head>
        ...
        <title>{{ __('Customer payment') }} {{ $paymentNo }}</title>
    ...
    <body>
        <div class="doc" id="boxes">
            <header class="doc-header">
                <h1 class="doc-title">{{ __('Customer payment') }}</h1>
                <div class="doc-brand">
                ...
                <div class="meta-right">
                    <table>
                        ...
                        @if ($linkedInvoice)
                            <tr>
                                <th>{{ __('Invoice') }}</th>
                                <td>{{ Utility::invoiceNumberFormat($settings, $linkedInvoice->invoice_id) }}</td>
                            </tr>
                        @endif
                    </table>
                </div>
