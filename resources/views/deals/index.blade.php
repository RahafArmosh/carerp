...
@section('action-btn')
    <div class="float-end">
         <form method="POST" action="{{ route('deals.change.pipeline') }}" id="change-pipeline" class="btn btn-sm">
        @csrf
                            ...
                            @foreach ($deals as $deal)
                                <div class="card" data-id="{{ $deal->id }}">
                                    ...
                                    <div class="card-header border-0 pb-0 position-relative">
                                        <h5><a
                                                href="@can('view deal')@if ($deal->is_active){{ route('deals.show', $deal->id) }}@else#@endif @else#@endcan">{{ $deal->name }}</a>
                                        </h5>
                                            ...
                                            @if (Auth::user()->type != 'client')
                                                <div class="btn-group card-option">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        ...
                                                        @endcan
                                                        @can('delete deal')
                                                            <form action="{{ route('deals.destroy', $deal->id) }}" method="POST" id="delete-form-{{ $deal->id }}">
                                                                @csrf
                                                                @method('DELETE')

                                                                <a href="#!" class="dropdown-item"
                                                                onclick="event.preventDefault(); if(confirm('{{ __('Are you sure? This action cannot be undone.') }}')) { document.getElementById('delete-form-{{ $deal->id }}').submit(); }">
