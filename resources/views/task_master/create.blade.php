<form action="{{ url('task-master') }}" method="post">
    @csrf
            ...
            <div class="col-12">
                <div class="form-group">
                    ...
                    <input type="text" name="name" id="name" class="form-control" value="{{ old('name') }}"
                        placeholder="{{ __('Task name') }}" required>
                </div>
            </div>
            <div class="col-12">
                <div class="form-group">
                    ...
                    <textarea name="description" id="description" class="form-control" rows="2"
                        placeholder="{{ __('Optional') }}">{{ old('description') }}</textarea>
                </div>
            </div>
            <div class="col-12">
                <div class="form-group">
                    ...
                    <select name="department_id" id="department_id" class="form-control select">
                        @foreach ($departments as $id => $label)
                            <option value="{{ $id === '' ? '' : $id }}"
                                {{ old('department_id') == $id ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    <small class="text-muted">{{ __('Leave empty to make the task available for all departments.') }}</small>
                </div>
            </div>
            <div class="col-12">
                <div class="form-group form-check">
                    <input type="checkbox" name="is_predefined" id="is_predefined" class="form-check-input" value="1"
                        {{ old('is_predefined', true) ? 'checked' : '' }}>
