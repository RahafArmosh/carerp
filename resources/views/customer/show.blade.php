...
@section('action-btn')
    <div class="float-end">
        ...
        @can('delete customer')
            <form method="POST" action="{{ route('customer.destroy', $customer['id']) }}" class="delete-form-btn">
                @csrf
                @method('DELETE')
...
@section('content')
            ...
            <div class="card customer-detail-box customer_card">
                <div class="card-body">
                    ...
                    <p class="card-text mb-0">
                        {{ $customer['chart_account_id'] != 0 ? \App\Models\ChartOfAccount::where('id', $customer['chart_account_id'])->first()->name : '-' }}
                    </p>
                </div>
            </div>
        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 col-sm-6">
                            ...
                            <div class="p-4">
                                ...
                                <h6 class="report-text mb-3">{{ \Auth::user()->priceFormat($customer->total_paid) }}</h6>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="card ">
        <div class="card-body employee-detail-body fulls-card">
            ...
            <div class="row">
                ...
                @endphp
                @if ($customer->accountingDocuments()->count() > 0)
                    <div class="col-md-6">
                        <div class="info text-sm">
                            <strong class="font-bold">Documents:</strong>
                            <ul>
                                @foreach ($customer->accountingDocuments as $document)
                                    <li>
                                        <a href="{{ URL::to('/') . '/' . $document->document_path }}" target="_blank"
                                            download>{{ $document->document_name }}</a>
                                        <div class="action-btn bg-danger ms-2">
                                            <form method="POST"
                                                action="{{ route('customer.file.destroy', $document->id) }}"
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body table-border-style table-border-style">
                    <h5 class="d-inline-block mb-5">{{ __('Pos') }}</h5>
                    <div class="table-responsive">
                        <table class="table">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                @foreach ($customer->customerPos($customer->id) as $posPayment)
                                    <tr>
                ...
                <div class="card-body table-border-style table-border-style">
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                @foreach ($customerPyment as $payment)
                                    <tr>
                                        {{--                                        <td>{{ AUth::user()->posNumberFormat($posPayment->pos_id) }}</td> --}}
                                        ...
                                        <td>
                                        @if (!empty($payment) && $payment->invoices()->count() > 0)
                                            @foreach ($payment->invoices as $invoice)
                                                ...
                                                @endif
                                            @endforeach
                                        @elseif (!empty($payment) && !empty($payment->invoice_id) && !empty($payment->invoice))
                                            {{-- Fallback for old single invoice relationship --}}
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="fileUploadForm" action="{{ route('upload.file.customer') }}" method="POST"
