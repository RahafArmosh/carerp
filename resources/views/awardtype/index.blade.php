                                        ...
                                        <td>
                                            ...
                                            @can('delete award type')
                                                <div class="action-btn bg-danger ms-2">
                                                    <form action="{{ route('awardtype.destroy', $awardtype->id) }}"
                                                        method="POST" id="delete-form-{{ $awardtype->id }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <a href="#"
                                                            class="mx-3 btn btn-sm align-items-center bs-pass-para"
                                                            data-bs-toggle="tooltip" title="{{ __('Delete') }}"
                                                            onclick="confirmDelete('{{ __('Are You Sure?') . '|' . __('This action can not be undone. Do you want to continue?') }}', 'delete-form-{{ $awardtype->id }}')">
