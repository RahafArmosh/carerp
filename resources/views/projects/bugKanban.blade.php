            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var html = '<div class="col-8 mb-2 file-' + data.id + '">' +
                        ...
                        data.file + '" class="btn btn-sm btn-primary">' +
                ...
                @foreach ($bugStatus as $status)
                    ...
                    <div class="col">
                        <div class="card">
                            ...
                            <div class="card-body kanban-box" id="task-list-{{ $status->id }}"
                                data-id="{{ $status->id }}">
                                @foreach ($bugs as $bug)
                                    <div class="card draggable-item" id="{{ $bug->id }}">
                                        <div class="pt-3 ps-3">
                                            @if ($bug->priority == 'low')
                                                <span
                                                    class="p-2 px-3 rounded badge badge-pill badge-xs bg-success">{{ ucfirst($bug->priority) }}</span>
                                            @elseif($bug->priority == 'medium')
                                                <span
                                                    class="p-2 px-3 rounded badge badge-pill badge-xs bg-warning">{{ ucfirst($bug->priority) }}</span>
                                            @elseif($bug->priority == 'high')
                                                <span
                                                    class="p-2 px-3 rounded badge badge-pill badge-xs bg-danger">{{ ucfirst($bug->priority) }}</span>
                                            @endif
                                        </div>
                                                    ...
                                                    @if (Gate::check('edit bug report') || Gate::check('delete bug report'))
                                                        <div class="dropdown-menu dropdown-menu-end">
                                                            ...
                                                            @can('delete project task')
                                                                <form
                                                                    action="{{ route('task.bug.destroy', [$project->id, $bug->id]) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('DELETE')

                                                                    <button type="submit" class="dropdown-item bs-pass-para">
                                            ...
                                            <div class="d-flex align-items-center justify-content-between mb-2">
                                                <ul class="list-inline mb-0">
                                                    <li class="list-inline-item d-inline-flex align-items-center"
                                                        data-bs-toggle="tooltip" title="{{ __('Start Date') }}">
                                                ...
                                                <div class="user-group">
                                                    <span data-bs-toggle="tooltip" title="{{ __('End Date') }}">
                                                        {{ \Auth::user()->dateFormat($bug->due_date) }}</span>
                                                </div>
                                            </div>
