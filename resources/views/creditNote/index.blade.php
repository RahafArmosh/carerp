                                    ...
                                    @if (!empty($invoice->creditNote))
                                        @foreach ($invoice->creditNote as $creditNote)
                                            <tr>
                                                ...
                                                <td>{{ \App\Models\ChartOfAccount::where('id',$creditNote->account_id)->first()->name }}</td>
                                                <td>{{ Auth::user()->priceFormat($creditNote->amount) }}</td>
                                                <td>{{ !empty($creditNote->description) ? $creditNote->description : '-' }}
                                                </td>
                                                <td>
                                                    ...
                                                    @can('edit credit note')
                                                        <div class="action-btn bg-danger ms-2">
                                                            <form method="POST"
                                                                ...
                                                                class="delete-form-btn" id="delete-form-{{ $creditNote->id }}">
                                                                {{ csrf_field() }}
                                                                ...
                                                                <a href="#"
                                                                    ...
                                                                    onclick="event.preventDefault(); if(confirm('{{ __('Are You Sure?') . ' ' . __('This action can not be undone. Do you want to continue?') }}')) { document.getElementById('delete-form-{{ $creditNote->id }}').submit(); }">
