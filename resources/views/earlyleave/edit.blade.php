<form action="{{ route('earlyleave.update', $leave->id) }}" method="POST">
    @csrf
    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select name="employee_id" id="employee_id" class="form-control select"
                            ...
                            @foreach ($employees as $key => $value)
                                <option value="{{ $key }}" @if ($key === $leave->employee_id) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    <textarea id="leave_reason" name="leave_reason" class="form-control" placeholder="{{ __('Leave Reason') }}">{{ $leave->reason }}</textarea>
                </div>
            </div>
        </div>
        @role('Company')
