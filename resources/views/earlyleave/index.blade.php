...
@section('page-title')
    {{ __('Manage Early Leave') }}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('Dashboard') }}</a></li>
    <li class="breadcrumb-item">{{ __('Manage Early Leave') }}</li>
@endsection

@section('action-btn')
    <div class="float-end">
        @can('create leave')
            <a href="#" data-size="lg" data-url="{{ route('earlyleave.create') }}" data-ajax-popup="true" data-bs-toggle="tooltip"
                title="{{ __('Create') }}" data-title="{{ __('Create Leave') }}" class="btn btn-sm btn-primary">
                            ...
                            <tbody>
                                @foreach ($leaves as $leave)
                                    <tr>
                                        ...
                                        <td>{{ \Auth::user()->dateFormat($leave->time) }}</td>
                                        <td>{{ $leave->reason }}</td>
                                        <td>
                                        ...
                                        <td>
                                                ...
                                                @if ($leave->status == 'Pending')
                                                    @can('edit leave')
                                                        <div class="action-btn bg-primary ms-2">
                                                            <a href="#"
                                                                data-url="{{ URL::to('earlyleave/' . $leave->id . '/edit') }}"
                                            ...
                                            @else
                                                <div class="action-btn bg-warning ms-2">
                                                    <a href="#"
                                                        data-url="{{ URL::to('earlyleave/' . $leave->id . '/action') }}"
                                                        data-size="lg" data-ajax-popup="true"
                                                ...
                                                @can('edit leave')
                                                    <div class="action-btn bg-primary ms-2">
                                                        <a href="#" data-url="{{ URL::to('earlyleave/' . $leave->id . '/edit') }}"
                                                            data-size="lg" data-ajax-popup="true"
                                                            data-title="{{ __('Edit Leave') }}"
                                                            class="mx-3 btn btn-sm  align-items-center" data-bs-toggle="tooltip"
                                                            title="{{ __('Edit') }}"
                                            ...
                                            @can('delete leave')
                                                <div class="action-btn bg-danger ms-2">
                                                    <form action="{{ route('earlyleave.destroy', $leave->id) }}" method="POST"
                                                        id="delete-form-{{ $leave->id }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <!-- Add your anchor tag or button to trigger the form submission -->
                                                        <a href="#"
