...
class EmployeeController extends Controller
{
    ...
    public function create()
    {
        if (\Auth::user()->can('create employee')) {
            ...
            $managers         = Employee::where('created_by', \Auth::user()->creatorId())->orderBy('name')->pluck('name', 'id');
            $managers->prepend('No Manager', '');
            ...
            return view('employee.create', compact('employees', 'employeesId', 'departments', 'designations', 'documents', 'branches', 'company_settings','chart_accounts', 'managers'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create employee')) {
            ...
            $startTime = $request->input('required_startTime'); // Example: '19:00'
            $endTime = $request->input('required_endTime'); // Example: '11:00'
            $employee = Employee::create(
                [
                    ...
                    'required_latitude' => $request['required_latitude'],
                    'required_longitude' => $request['required_longitude'],
                    'created_by' => \Auth::user()->creatorId(),
                    'startTime' => Carbon::createFromFormat('H:i', $startTime)->format('h:i A'),
                    'endTime' => Carbon::createFromFormat('H:i', $endTime)->format('h:i A'),
                    'chart_account_id' => !empty($request->chart_account) ? $request->chart_account : \App\Models\ChartOfAccount::where('code', 1200)->first()->id,
                    'employee_device_id' => $request->filled('employee_device_id') ? trim((string) $request->employee_device_id) : null,
                ]
            );
            ...
            if ($request->hasFile('document')) {
                foreach ($request->document as $key => $document) {
                    ...
                    $document->move(public_path('documents/employee'), $fileNameToStore);
    ...
    public function edit($id)
    {
        ...
        if (\Auth::user()->can('edit employee')) {
            ...
            $managers     = Employee::where('created_by', \Auth::user()->creatorId())
            ...
            ->pluck('code_name', 'id');
            $chart_accounts->prepend('Select Account', '');
            return view('employee.edit', compact('employee', 'employeesId', 'branches', 'departments', 'designations', 'documents', 'departmentData','chart_accounts', 'managers'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (\Auth::user()->can('edit employee')) {
            ...
            if ($request->filled('manager_id') && $this->createsManagerCycle($employee, (int) $request->manager_id)) {
                return redirect()->back()->withInput()->with('error', __('Invalid manager selection. Circular reporting is not allowed.'));
            }
            ...
            $employee = Employee::findOrFail($id);
            $input    = $request->all();
            $input['manager_id'] = $request->filled('manager_id') ? (int) $request->manager_id : null;
            $employee->fill($input)->save();
            $employee->employee_device_id = $request->filled('employee_device_id')
                ? trim((string) $request->employee_device_id)
                : null;
            $employee->chart_account_id = $request->chart_account_id;
            $employee->startTime = Carbon::createFromFormat('H:i', $startTime)->format('h:i A');
            $employee->endTime = Carbon::createFromFormat('H:i', $endTime)->format('h:i A');
            $employee->save();
            $employee = Employee::find($id);
    ...
    private function createsManagerCycle(Employee $employee, int $managerId): bool
    {
        $visitedIds = [$employee->id];
        $currentId = $managerId;
