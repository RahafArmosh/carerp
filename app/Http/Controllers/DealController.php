...
class DealController extends Controller
{
    ...
    {
        $usr      = \Auth::user();
        $hasCrmAdminAccess = $usr->can('manage crm admin');

        if ($usr->can('manage deal') || $hasCrmAdminAccess) {
            ...
            $curr_month  = Deal::where('pipeline_id', '=', $pipeline?->id)->whereMonth('created_at', '=', date('m'))->get();
            $curr_week   = Deal::where('pipeline_id', '=', $pipeline?->id)->whereBetween(
                'created_at',
                [
            ...
            $last_30days = Deal::where('pipeline_id', '=', $pipeline?->id)->whereDate('created_at', '>', \Carbon\Carbon::now()->subDays(30))->get();
            // Deal Summary
            $cnt_deal                = [];
    ...
    public function deal_list(Request $request)
    {
        ...
        if ($usr->can('manage deal') || $hasCrmAdminAccess) {
            ...
            if ($usr->type == 'client') {
                $id_deals = $usr->clientDeals->pluck('id');
            } elseif ($usr->type == 'company' || $hasCrmAdminAccess) {
                $id_deals = Deal::where('created_by', $usr->ownerId())->pluck('id');
            } else {
                $userId = !empty($request->user) ? User::find($request->user_id) : $usr;
                $id_deals = $usr->deals->pluck('id');
            }
            ...
            // Deals
            if ($usr->type == 'client') {
                $deals = Deal::select('deals.*')->join('client_deals', 'client_deals.deal_id', '=', 'deals.id')->where('client_deals.client_id', '=', $usr->id)->where('deals.pipeline_id', '=', $pipeline->id)->orderBy('deals.order')->paginate($perPage);
            } elseif ($usr->type == 'company' || $hasCrmAdminAccess) {
                $deals = Deal::select('deals.*')->where('deals.pipeline_id', '=', $pipeline->id)->where('created_by', $usr->ownerId())->orderBy('deals.created_at')->paginate($perPage);
            } elseif ($usr->type == 'manager') {
                // Get IDs of users managed by this manager
                $managedUserIds = \App\Models\User::where('manager_id', $usr->id)->pluck('id');
                // Include the manager's own ID
                $allUserIds = $managedUserIds->push($usr->id);
                ...
                $deals = Deal::select('deals.*')->join('user_deals', 'user_deals.deal_id', '=', 'deals.id')->where('user_deals.user_id', '=', $usr->id)->where('deals.pipeline_id', '=', $pipeline->id)->orderBy('deals.created_at')->paginate($perPage)->appends(request()->query());
            }
            if (!empty($request->user_id)) {
                $deals = Deal::select('deals.*')->join('user_deals', 'user_deals.deal_id', '=', 'deals.id')->where('user_deals.user_id', '=', $request->user_id)->where('deals.pipeline_id', '=', $pipeline->id)->orderBy('deals.created_at')->paginate($perPage)->appends(request()->query());
            }
    ...
    public function store(Request $request)
    {
        ...
        if ($usr->can('create deal')) {
            ...
            if (empty($stage)) {
                ...
                if ($usr->type == 'company' || $hasCrmAdminAccess) {
                    $usrDeals = [
                        $usr->id,
    ...
    public function edit(Deal $deal)
    {
        if (\Auth::user()->can('edit deal')) {
            if ($deal->created_by == \Auth::user()->ownerId()) {
                ...
                $stageCnt      = Stage::where('pipeline_id', '=', $deal->pipeline_id)->where('created_by', '=', $deal->created_by)->get();
                return view('deals.edit', compact('deal', 'pipelines', 'sources', 'products', 'customFields', 'stageCnt'));
            } else {
                return response()->json(['error' => __('Permission Denied.')], 401);
            }
        } else {
    ...
    public function update(Request $request, Deal $deal)
    {
        if (\Auth::user()->can('edit deal')) {
            if ($deal->created_by == \Auth::user()->ownerId()) {
                // if ($request->has('notes') || $request->has('files')) {
                ...
                $deal->SOID    = $request->SOID;
                $deal->payment    = $request->payment;
        ...
        if (\Auth::user()->can('edit deal')) {
            ...
            if ($deal->created_by == \Auth::user()->ownerId()) {
                $products = ProductService::where('created_by', \Auth::user()->creatorId())
                    ->whereNotIn('id', explode(',', $deal->products))
                ...
                return view('deals.products', compact('deal', 'products', 'currencies'));
            } else {
                return response()->json(['error' => __('Permission Denied.')], 401);
            }
        } else {
    ...
    public function productUpdate($id, Request $request)
    {
        ...
        if (!$deal || $deal->created_by != $user->creatorId()) {
            return redirect()->back()->with('error', __('Permission Denied.'))->with('status', 'products');
        }
        ...
        foreach ($request->products as $productData) {
            if (isset($productData['id']) && isset($productData['quantity']) && $productData['quantity'] > 0) {
                ...
                $currencyId = isset($productData['currency_id']) && !empty($productData['currency_id']) ? $productData['currency_id'] : null;
                $exchangeRate = isset($productData['exchange_rate']) && !empty($productData['exchange_rate']) ? $productData['exchange_rate'] : null;
                $exchangePrice = null;
                
                // Handle currency conversion
                if ($currencyId) {
                    // If exchange rate not provided, get from currency
                    if (!$exchangeRate) {
                        $currency = Currency::find($currencyId);
                        $exchangeRate = $currency ? $currency->exchange_rate : 1;
                    }
                    
                    // Store the price in the selected currency as exchange_price
                    $exchangePrice = $price;
                    
                    // Convert to base currency for storage in price field
                    $price = $exchangePrice * $exchangeRate;
                }

                // Save to deal_products table
                DealProduct::create([
                    'deal_id'    => $deal->id,
        ...
        $existingProductIds = !empty($deal->products) ? explode(',', $deal->products) : [];
        $allProductIds = array_unique(array_merge($existingProductIds, $newProductIds));
        $deal->products = implode(',', $allProductIds);
        $deal->save();
    ...
    public function productDestroy($id, $uid)
    {
        if (!\Auth::user()->can('edit deal')) {
            return redirect()->back()->with('error', __('Permission Denied.'))->with('status', 'products');
        }
        ...
        $deal = Deal::find($id);
        if (!$deal || $deal->created_by != \Auth::user()->creatorId()) {
            return redirect()->back()->with('error', __('Permission Denied.'))->with('status', 'products');
        }

        // Find the DealProduct by ID (uid is the DealProduct ID)
        $dealProduct = DealProduct::where('deal_id', $id)
            ->where('id', $uid)
            ->first();

        if (!$dealProduct) {
            return redirect()->back()->with('error', __('Product not found.'))->with('status', 'products');
        }

        $productId = $dealProduct->product_id;

        // Remove from deal_products table
        $dealProduct->delete();

        // Update the comma-separated field in the deals table
        $productIds = !empty($deal->products) ? explode(',', $deal->products) : [];
        $productIds = array_filter($productIds, function ($pid) use ($productId) {
            return (int)$pid !== (int)$productId;
        });

        $deal->products = implode(',', $productIds);
        $deal->save();
        ...
        if (\Auth::user()->can('edit deal')) {
            ...
            if ($deal->created_by == \Auth::user()->ownerId()) {
                ...
                try {
                    $usr = \Auth::user();
                    Notification::create([
                        'user_id'    => \Auth::user()->manager_id ?? \Auth::user()->creatorId(),
                        'type'       => 'Deal_uploaded_file',
                        'created_by' => \Auth::user()->id,
                        'data'       => [
                            'lead_id' => $deal->lead->id,
                            'deal_id' => $deal->id,
                            'message' => "{$usr->name} has uploaded an attachment to the deal {$deal->name}"
                        ]
                    ]);
                } catch (\Exception $e) {
                    dd($e->getMessage());
    ...
    public function fileDownload($id, $file_id)
    {
        if (\Auth::user()->can('edit deal')) {
            ...
            if ($deal->created_by == \Auth::user()->ownerId()) {
                ...
                if ($file) {
                    $file_path = storage_path('app/public/deal_files/' . $file->file_path);
    ...
    public function editProduct($dealId, $productId)
    {
        $deal = Deal::findOrFail($dealId);
        $product = $deal->dealProducts()->where('id', $productId)->firstOrFail();
        $currencies = Currency::get()->pluck('name', 'id');

        return view('deals.edit_product', compact('deal', 'product', 'currencies'));
    }

    public function updateProduct(Request $request, $dealId, $productId)
    {
        $request->validate([
        ...
        $exchangeRate = $request->exchange_rate ?? null;
        $exchangePrice = null;
        ...
        return redirect()->route('deals.show', $dealId)->with('success', 'Product updated successfully.');
    }
