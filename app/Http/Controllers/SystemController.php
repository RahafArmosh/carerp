        ...
        if (\Auth::user()->type == 'company') {
            $request->validate(
                [
                    ...
                    'mail_from_address' => 'string|max:255',
    ...
    public function saveBusinessSettings(Request $request)
    {
        ...
        if (\Auth::user()->can('manage business settings')) {
            ...
            if ($request->company_logo_dark) {
                ...
                $path = Storage::putFileAs('public/uploads/logo', $file, $logoName);
                $file->move(public_path('documents'), $logoName);
            ...
            if ($request->company_favicon) {
                ...
                $path = Storage::putFileAs('public/uploads/logo', $file, $favicon);
                $file->move(public_path('documents'), $favicon);
            ...
            if ($request->company_stamp) {
                ...
                //                $favicon = 'favicon.png';
                $companyStamp = $user->id . '-stamp-preview.png';

                $dir = 'uploads/logo/';
                $file = $request->file('company_stamp');
                $path = Storage::putFileAs('public/uploads/logo', $file, $companyStamp);
                $file->move(public_path('documents'), $companyStamp);
            ...
            if (
                ...
                unset($post['_token'], $post['company_logo_dark'], $post['company_logo_light'], $post['company_favicon'], $post['company_stamp']);
                foreach ($post as $key => $data) {
                    if (in_array($key, array_keys($settings))) {
