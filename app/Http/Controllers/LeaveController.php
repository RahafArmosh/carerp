...
class LeaveController extends Controller
{
    ...
    public function request_leave(Request $request)
    {
        ...
        if ($leave_type->days >= $total_leave_days) {
            $leave    = new Leave();
            $leave->employee_id = $employee->idAuth::id();
            $leave->leave_type_id    = $request->leave_type_id;
            $leave->applied_on       = date('Y-m-d');
            $leave->start_date       = $request->start_date;
            $leave->end_date         = $request->end_date;
            ...
            return response()->json('Leave type ' . $leave_type->title . ' is provide maximum ' . $leave_type->days . "  days please make sure your selected days is under " . $leave_type->days . ' days.', 201);
        }
    }
    public function request_my_leave()
    {
        $employee = Employee::where('user_id', '=', Auth::user()->id)->first();
        $leaves = Leave::where('employee_id', '=',  $employee->id)->with(['leaveType', 'employees'])->get();
        return response()->json($leaves, 200);
    }
    public function update_leave(Request $request, $leave)
    {
        ...
        if ($leave->created_by == Auth::id()) {
            ...
            if ($leave_type->days >= $total_leave_days) {
                ...
                return response()->json(['message' => 'Leave successfully updated.'], 201);
            } else {
                return response()->json('Leave type ' . $leave_type->title . ' is provide maximum ' . $leave_type->days . "  days please make sure your selected days is under " . $leave_type->days . ' days.', 201);
            }
        } else {
            return response()->json(__('Permission denied.'), 200);
        }
    }
}
