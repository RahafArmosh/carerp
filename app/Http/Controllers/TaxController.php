...
class TaxController extends Controller
{
    ...
    public function create()
    {
        if (\Auth::user()->can('create constant tax')) {
            ...
            return view('taxes.create', compact('chart_accounts'));
        } else {
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create constant tax')) {
            ...
            $tax->chart_account_id = !empty($request->chart_account) ? $request->chart_account : 0;
            $tax->save();
    ...
    public function edit(Tax $tax)
    {
        if (\Auth::user()->can('edit constant tax')) {
            if ($tax->created_by == \Auth::user()->creatorId()) {
                ...
                $tax_chart_accounts = ChartOfAccount::where('id', $tax->chart_account_id)->first();
                return view('taxes.edit', compact('tax', 'chart_accounts', 'tax_chart_accounts'));
            } else {
                return response()->json(['error' => __('Permission denied.')], 401);
            }
        } else {
