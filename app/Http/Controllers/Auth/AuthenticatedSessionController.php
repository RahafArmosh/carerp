...
class AuthenticatedSessionController extends Controller
{
    ...
    public function store(LoginRequest $request)
    {
        ...
        if($user != null && $user->is_disable == 0)
        {
            return redirect()->back()->with('status', __('Your Account is disable,please contact your Administrator.'));
        }

        // Check if user is API-only (prevent web login)
        if ($user && $this->isApiOnlyUser($user)) {
            return redirect()->back()->with('status', __('This account can only be accessed via API. Please use the API login endpoint.'));
        }
...
//        if($user->type =='employee')
        ...
        {
            // Check what dashboard permissions the user has and redirect accordingly
            // This prevents redirect loops when user doesn't have the default dashboard permission
            if($user->can('show pos dashboard'))
            {
            ...
            {
                return redirect()->intended(route('project.dashboard'));
            }
            elseif($user->can('show account dashboard'))
            {
                return redirect()->intended(RouteServiceProvider::HOME);
            }
            else
            {
                // User has no dashboard permissions - redirect to profile page
                return redirect()->intended(route('profile'))->with('error', __('You do not have permission to access any dashboard. Please contact your administrator to assign the necessary permissions.'));
            }
        }
    ...
    /**
     * Check if user is API-only (should not be able to login via web)
     * 
     * @param User $user
     * @return bool
     */
    private function isApiOnlyUser($user)
    {
        // Check database field first (preferred method)
        if (isset($user->api_only) && $user->api_only == true) {
            return true;
        }
        
        // Fallback: Check hardcoded email list for backward compatibility
        $apiOnlyEmails = [
            'saharamallmanagement@luxemarca.com',
            // Add more API-only emails here if needed
        ];
        
        // Check if user email is in the API-only list
        return in_array(strtolower($user->email), array_map('strtolower', $apiOnlyEmails));
    }
