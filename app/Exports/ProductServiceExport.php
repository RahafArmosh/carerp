...
class ProductServiceExport implements FromCollection, WithHeadings
{
    ...
    public function collection()
    {
        ...
        $data = ProductService::select('product_services.id', 'product_services.name as item',  'sku', 'sale_price', 'purchase_price', 'tax_id as tax', 'product_service_categories.name as category', 'brands.name as brand', 'sub_brands.name as sub_brand', 'product_service_units.name as unit', 'product_services.type', 'description');
//        $data->leftjoin('venders', 'product_services.assign_vendor', '=', 'venders.id');
        ...
        $data->leftjoin('brands', 'product_services.brand_id', '=', 'brands.id');
        $data->leftjoin('sub_brands', 'product_services.sub_brand_id', '=', 'sub_brands.id');
