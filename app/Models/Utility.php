...
class Utility extends Model
{
    ...
    public static function settings()
    {
        ...
        $settings = [
            "site_currency" => "AED",
            "site_currency_symbol" => "Dhs",
            "site_currency_symbol_position" => "post",
            "site_vat_calculation" => "not_add",
...
',
            ...
            'mail_port' => '587',
            'mail_username' => 'falconsgtit@gmail.com',
            'mail_password' => 'FalconsGT123', // Not your main Gmail password!
            'mail_encryption' => 'tls',
            'mail_from_address' => 'falconsgtit@gmail.com',
            'mail_from_name' => 'ERP',
    ...
    public static function templateData()
    {
        ...
        // Separate template lists per module
        $arr['templates'] = [
            'proposal' => [
                'template1' => 'Classic',
                'template11' => 'Minimal (PDF)',
            ],
            'invoice' => [
                'template1' => 'Classic',
                'template11' => 'Minimal (PDF)',
        ...
        // Descriptions per module & per template
        $arr['descriptions'] = [
            'proposal' => [
                'template1' => 'Full proposal layout with company branding, line items, totals, and terms.',
                'template11' => 'Clean black-and-white layout matching the bill minimal template: proposal number, company name and email under the title; customer block and dates; line items as Description / Rate / Qty / Total; footer shows company address and stamp from company settings.',
            ],
            'invoice' => [
                'template1' => 'Full invoice layout with company branding, line items, tax and discount breakdown, and bank/terms sections.',
                'template11' => 'Clean black-and-white layout matching the bill minimal template: invoice number, company name and email under the title; customer block and dates; line items as Description / Rate / Qty / Total; paid, credit note, and due; footer shows company address and stamp from company settings.',
            ],
            'bill' => [
                'template11' => 'Clean black-and-white layout: bill number, company name and email under the title; vendor block and dates; line items as Description / Rate / Qty / Total; footer shows company address and stamp from company settings.',
            ],
        ];
        return $arr;
    }
    ...
    /**
     * Map removed bill print templates to a supported template.
     */
    public static function resolveBillTemplate(?string $template): string
    {
        $t = $template ?: 'template1';
        if (in_array($t, ['template3', 'template4'], true)) {
            return 'template11';
        }
    ...
    /**
     * Map removed proposal print templates (Rio / London) to the minimal template.
     */
    public static function resolveProposalTemplate(?string $template): string
    {
        $t = $template ?: 'template1';
        if (in_array($t, ['template3', 'template4'], true)) {
            return 'template11';
    ...
    /**
     * Map removed invoice print templates (Rio / London) to the minimal template.
     */
    public static function resolveInvoiceTemplate(?string $template): string
    {
        $t = $template ?: 'template1';
        if (in_array($t, ['template3', 'template4'], true)) {
            return 'template11';
    ...
    public static function priceNumberFormat($settings, $price)
    {
        return number_format($price, $settings['decimal_number']);
    }

    public static function priceFormatCurr($settings, $price, $site_currency_symbol)
    {

        return (($settings['site_currency_symbol_position'] == "pre") ? $site_currency_symbol : '') . number_format($price, $settings['decimal_number']) . (($settings['site_currency_symbol_position'] == "post") ? $site_currency_symbol : '');
    }
    public static function formatPriceToWords($number)
    {
        $f = new \NumberFormatter("en", \NumberFormatter::SPELLOUT);

        // Round the number to avoid long decimals
        $roundedNumber = round($number, 2);

        // Format the rounded number
        $formatted = $f->format($roundedNumber);
    ...
    public static function getCrmPercentage($val1 = 0, $val2 = 0)
    {
        ...
        if ($val1 > 0 && $val2 > 0) {
            ...
            $percentage = number_format($percentage, (int)\Utility::getValByName('decimal_number'));
        }
    ...
    public static function getAccountBalance($account_id, $start_date = null, $end_date = null)
    {
        ...
        $total->leftjoin('chart_of_accounts', 'general_ledger.account', 'chart_of_accounts.id');
        ...
        $total->where('general_ledger.created_by', \Auth::user()->creatorId());
        $total->where('general_ledger.account', $account_id);
        $total->where('general_ledger.send_date', '>=', $start);
        $total->where('general_ledger.send_date', '<=', $end);
        $total->groupBy('account');
    ...
    public static function getAccountBalanceNew($account_id)
    {
        $total = GeneralLedger::select(
            'chart_of_accounts.id',
            'chart_of_accounts.code',
            ...
            ->where('general_ledger.account', $account_id)
            ->groupBy('chart_of_accounts.id', 'chart_of_accounts.code', 'chart_of_accounts.name')
            ->first(); // no need for ->get()->toArray(), we only need one row
        ...
        return (float) $total->totalDebit - (float) $total->totalCredit;
    }


    public static function getAccountDebit($account_id)
    {
        // foreach ($types as $type) {
        $total = GeneralLedger::select(
            'chart_of_accounts.id',
    ...
    public static function getAccountCredit($account_id)
    {
        // foreach ($types as $type) {
        $total = GeneralLedger::select(
            'chart_of_accounts.id',
    ...
    public static function getOpenAccountBalance($accountId, $userId = null)
    {
    ...
    public static function getAccountData($account_id, $start_date = null, $end_date = null)
    {
        ...
        $transactionData = DB::table('general_ledger')
            ->where('general_ledger.created_by', \Auth::user()->creatorId())
            ->where('general_ledger.account', $account_id)
            ->whereBetween('general_ledger.send_date', [$start, $end])
            ->leftJoin('invoices', function ($join) {
                $join->on('general_ledger.ref_id', '=', 'invoices.id')
                    ->whereIn('general_ledger.reference', ['Invoice Payment', 'Invoice', 'Delete Invoice', 'Invoice Delete Product']);
            })
            ->leftJoin('bills', function ($join) {
                $join->on('general_ledger.ref_id', '=', 'bills.id')
                    ->whereIn('general_ledger.reference', ['Bill', 'Delete Bill', 'Bill Payment', 'Bill Account', 'Expense', 'Expense Account', 'Expense Payment', 'Delete Expense', 'Delete Item From Bill']);
            })
            ->leftJoin('revenues', function ($join) {
                $join->on('general_ledger.ref_id', '=', 'revenues.id')
                    ->whereIn('general_ledger.reference', ['Revenue', 'Delete Revenue']);
            })
            ->leftJoin('payments', function ($join) {
                $join->on('general_ledger.ref_id', '=', 'payments.id')
                    ->whereIn('general_ledger.reference', ['Payment', 'Delete Bill Payment', 'Delete Payment', 'Bill Payment']);
            })
            ->leftJoin('customer_payments', function ($join) {
                $join->on('general_ledger.ref_id', '=', 'customer_payments.id')
                    ->whereIn('general_ledger.reference', ['Customer Payment', 'Delete Customer Payment', 'Invoice Payment', 'Delete Invoice Payment']);
            })
            ->leftJoin('customer_refunds', function ($join) {
                $join->on('general_ledger.ref_id', '=', 'customer_refunds.id')
                    ->whereIn('general_ledger.reference', ['Customer Refund', 'Delete Customer Refund']);
            })
            ->leftJoin('credit_notes', function ($join) {
                $join->on('general_ledger.ref_id', '=', 'credit_notes.id')
                    ->whereIn('general_ledger.reference', ['Credit Note', 'Delete Credit Note']);
            })
            ->leftJoin('debit_notes', function ($join) {
                $join->on('general_ledger.ref_id', '=', 'debit_notes.id')
                    ->whereIn('general_ledger.reference', ['Debit Note', 'Delete Debit Note']);
            })
            ->leftJoin('bank_transfers', function ($join) {
                $join->on('general_ledger.ref_id', '=', 'bank_transfers.id')
                    ->whereIn('general_ledger.reference', ['Bank Transfer', 'Delete Bank Transfer']);
            })
            ->leftJoin('journal_entries', function ($join) {
                $join->on('general_ledger.ref_id', '=', 'journal_entries.id')
                    ->whereIn('general_ledger.reference', ['Journal Entries', 'Delete Journal Entries']);
            })
            ->leftJoin('pos', function ($join) {
                $join->on('general_ledger.ref_id', '=', 'pos.id')
                    ->whereIn('general_ledger.reference', ['POS', 'Delete POS']);
            })
            ->leftJoin('refunds', function ($join) {
                $join->on('general_ledger.ref_id', '=', 'refunds.id')
                    ->whereIn('general_ledger.reference', ['Bill Refund', 'Vendor Refund', 'Delete Bill Refund', 'Delete Vendor Refund']);
            })
            ->leftJoin('customers as revenues_customers', function($join) {
                $join->on('revenues.customer_id', '=', 'revenues_customers.id')
                     ->where('revenues_customers.created_by', '=', \Auth::user()->creatorId());
            })
            ->leftJoin('venders as payments_venders', function($join) {
                $join->on('payments.vender_id', '=', 'payments_venders.id')
                     ->where('payments_venders.created_by', '=', \Auth::user()->creatorId());
            })
            ->leftJoin('customers', function($join) {
                $join->on('invoices.customer_id', '=', 'customers.id')
                     ->where('customers.created_by', '=', \Auth::user()->creatorId());
            })
            ->leftJoin('venders', function($join) {
                $join->on('bills.vender_id', '=', 'venders.id')
                     ->where('venders.created_by', '=', \Auth::user()->creatorId());
            })
            ->leftJoin('chart_of_accounts', 'general_ledger.account', '=', 'chart_of_accounts.id')
            // Join user tables based on user_id (not ref_id)
            ->leftJoin('customers as gl_customers', function($join) {
                $join->on('general_ledger.user_id', '=', 'gl_customers.id')
                     ->where('gl_customers.created_by', '=', \Auth::user()->creatorId());
            })
            ->leftJoin('venders as gl_venders', function($join) {
                $join->on('general_ledger.user_id', '=', 'gl_venders.id')
                     ->where('gl_venders.created_by', '=', \Auth::user()->creatorId());
            })
            ->leftJoin('employees as gl_employees', function($join) {
                $join->on('general_ledger.user_id', '=', 'gl_employees.id')
                     ->where('gl_employees.created_by', '=', \Auth::user()->creatorId());
            })
            ->leftJoin('users as gl_users', function($join) {
                $join->on('general_ledger.user_id', '=', 'gl_users.id')
                     ->where('gl_users.created_by', '=', \Auth::user()->creatorId());
            })
            ->select(
                'general_ledger.*',
                ...
                DB::raw("CASE 
                    WHEN LOWER(chart_of_accounts.name) = 'account payable' THEN 
                        COALESCE(gl_venders.name, venders.name, payments_venders.name, gl_customers.name, gl_employees.name, gl_users.name, customers.name, revenues_customers.name)
                    WHEN LOWER(chart_of_accounts.name) = 'account receivables' THEN 
                        COALESCE(gl_customers.name, customers.name, revenues_customers.name, gl_venders.name, gl_employees.name, gl_users.name, venders.name, payments_venders.name)
                    ELSE 
                        COALESCE(gl_customers.name, gl_venders.name, gl_employees.name, gl_users.name, customers.name, venders.name, revenues_customers.name, payments_venders.name)
                END as user_name"),
    ...
    public static function getPreviousAccountBalance($account_id, $start_date)
    {
        return GeneralLedger::where('general_ledger.account', $account_id)
            ->where('general_ledger.created_by', \Auth::user()->creatorId())
            ->where('general_ledger.send_date', '<', $start_date)
            ->selectRaw('SUM(debit) as totalDebit, SUM(credit) as totalCredit')
            ->first()
            ->totalDebit - (GeneralLedger::where('general_ledger.account', $account_id)
                ->where('general_ledger.created_by', \Auth::user()->creatorId())
                ->where('general_ledger.send_date', '<', $start_date)
                ->selectRaw('SUM(credit) as totalCredit')
                ->first()->totalCredit ?? 0);
    }
