...
class User extends Authenticatable //implements MustVerifyEmail
{
    ...
    public function getProfileAttribute()
    {
        ...
        if (!empty($this->avatar) && file_exists(storage_path('app/' . $this->avatar))) {
            return $this->attributes['avatar'] = asset('storage/' . $this->avatar);
        } else {
            return $this->attributes['avatar'] = asset('storage/avatar.png');
        }
    }
    ...
    {
        // If price is already a formatted string, extract numeric value
    ...
    public function priceFormatCurr($price,$currency)
    {

        $settings = Utility::settings();
        $decimal_number = Utility::getValByName('decimal_number') ? Utility::getValByName('decimal_number') : 0;

        return (($settings['site_currency_symbol_position'] == "pre") ? $currency : '') . number_format($price, $decimal_number) . (($settings['site_currency_symbol_position'] == "post") ? $currency : '');
    }
    ...
    public function proNumberFormat($number)
    {
        return "#PRO" . sprintf("%05d", $number);
    }

    public function asnNumberFormat($number)
    {
        return "ASN" . sprintf("%05d", $number);
    }

    public function saleOrderNumberFormat($number)
    {
        return "SO" . sprintf("%04d", $number);
    }

    public function packingListNumberFormat($number)
    {
        return "PL" . sprintf("%05d", $number);
    }

    public function revenueNumberFormat($number)
    {
        $settings = Utility::settings();

        return '#RV' . sprintf("%05d", $number);
    }

    public function manufacturerNumberFormat($number)
    {
        $settings = Utility::settings();

        return '#MAN' . sprintf("%05d", $number);
    }
    ...
    public function paymentNumberFormat($number)
    {
        ...
        return 'VP' . sprintf("%05d", $number);
    }

    public function CustomerpaymentNumberFormat($number)
    {
        $settings = Utility::settings();

        return 'CP' . sprintf("%05d", $number);
    }

    public function paymentNumberRefundFormat($number)
    {
        $settings = Utility::settings();

        return 'VR' . sprintf("%05d", $number);
    }
    ...
    public function todayExpense()
    {
        ...
        $totalExpense = (!empty($billTotal) ? ($billTotal) : 0);
        ...
        if ($month != '' && $date != '') {
            $InvoiceProducts = \DB::table('invoice_products')
                ->select(
                    ...
                    \DB::raw('SUM(price)  as sub_total')
                )
                ->selectRaw('(SELECT SUM((price - discount) * (taxes.rate / 100)) FROM invoice_products
    ...
    public static function getBillProductsData($date = '', $month = '')
    {
        if ($month != '' && $date != '') {
            $BillProducts = \DB::table('bill_products')
                ->select(
                    ...
                    \DB::raw('SUM(bill_products.price)  as sub_total')
                )
                ->selectRaw('(SELECT SUM(bill_accounts.price) FROM bill_accounts
                    WHERE bill_accounts.ref_id = bills.id) as acc_price')
                ->selectRaw('(SELECT SUM((price  - discount) * (taxes.rate / 100)) FROM bill_products
                ...
                ->leftJoin('chart_of_accounts', 'product_service_categories.purchase_account_id', 'chart_of_accounts.id')
                ->where(\DB::raw('YEAR(bills.send_date)'), '=', $date)
                ->where(\DB::raw('MONTH(bills.send_date)'), '=', $month)
                ->whereIN('chart_of_accounts.type', [5, 6]) // Adding condition for type = 'expenses'
        ...
        $BillProducts->map(function ($bill) {
            $bill->total = $bill->sub_total - $bill->total_discount;
            return $bill;
        });
        $total = 0;
    ...
    public function billsData($start, $current)
    {
        $billProducts = Bill::select('bills.bill_id as bill')
            ->selectRaw('sum((bill_products.price) - bill_products.discount) as price')
        ...
        foreach ($mergedArray as $bill) {
            $billTotal += $bill['price'] + $bill['total_tax'];
            $billPaid += $bill['pay_price'] + $bill['debit_price'];
            $billDue += ($bill['price'] + $bill['total_tax']) - $bill['pay_price'] - $bill['debit_price'];
        }
    ...
    {
        return $this->belongsToMany('App\Models\warehouse', 'user_warehouses', 'user_id', 'warehouse_id');
    }
    ...
    public static function userDefaultWarehouse(int $companyUserId)
    {
        ...
        if (! $countryId) {
            CompanyCurrencyCountryDefaultsSeeder::seedForCompany($companyUserId);
            $countryId = Country::withoutGlobalScopes()
                ->where('created_by', $companyUserId)
                ->orderBy('id')
                ->value('id');
    ...
    public function userWarehouseRegister($user_id)
    {
        ...
        if (! $countryId) {
            CompanyCurrencyCountryDefaultsSeeder::seedForCompany((int) $user_id);
            $countryId = Country::withoutGlobalScopes()
                ->where('created_by', $user_id)
                ->orderBy('id')
                ->value('id');
