...
function show_toastr(type, message) {
    ...
    // If message contains HTML, use it as-is, otherwise convert newlines to <br>
    if (formattedMessage.indexOf('<') === -1 && formattedMessage.indexOf('>') === -1) {
        formattedMessage = formattedMessage.replace(/\n/g, '<br>');
    }
    $("#liveToast .toast-body").html(formattedMessage);
}
...
$(document).on(
    ...
    function () {
        ...
        // Clear modal body before loading new content to prevent showing cached/stale data
        ...
        if ($("#tax_hidden").length > 0) {
            data["tax_id"] = $("#tax_hidden").val();
        }
        
        // Add cache-busting parameter to ensure fresh content is loaded (only if URL doesn't already have query params)
        if (url.indexOf('?') === -1) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (!data || data.trim() === '') {
                    ...
                    show_toastr("Error", "Empty response from server", "error");
                    return;
                }
...
$(document).on(
    "click",
    ".local_calender .fc-daygrid-event, .fc-timegrid-event",
    function (e) {
...
$(document).on(
    ...
    function () {
        ...
        var size_over =
            $(this).data("size") == "" ? "md" : $(this).data("size");
...
// SweetAlert for delete buttons that target a hidden form by ID
$(document).on("click", ".delete-subproduct-btn", function () {
